...
library(devtools)
devtools::install_gitlab("data-analysis5/phenotypes/process.phenotypes@default", auth_token = devtools::github_pat(), build_vignettes=TRUE)
```
...
```
library(devtools)
devtools::install("/path/to/process.phenotypes", build_vignettes=TRUE)
```
...
## Method 4: Installation from CRAN

We are in the process of submitting the `process.phenotypes` package to CRAN

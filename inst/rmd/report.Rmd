...
knitr::kable(process.phenotypes:::get.top.ten(FALSE, subjects.wrong.type, "Subjects"), caption = "Ten lowest type conversion failures", table.attr = knitr.table.attr)
```
...
knitr::kable(process.phenotypes:::get.top.ten(FALSE, variables.wrong.type, "Variables"), caption = "Ten lowest type conversion failures", table.attr = knitr.table.attr)
```
...
for (name in names(variable.summary$variables)) {
  ...
  if (is.vector(phenotype.data[, name], mode = "numeric")) {
    if (length(which(!is.na(phenotype.data[, name])))) {
      ...
      if (!is.null(variable.summary$variables[[name]]$params$multimodal)) {
        ...
        hist.plot.colours <- RColorBrewer::brewer.pal(length(multimodal.values), "Dark2")
  ...
  if (is.vector(phenotype.data[, name], mode = "numeric") |
    ...
    length(my.summ) < nrow(phenotype.data) * unique.variable.value.inclusion.proportion) {
  ...
  if (!is.null(variable.summary$variables[[name]]$params$subject_ancestry)) {
    ...
    ## improved
    df <- data.frame(
      Input = variable.summary$variables[[name]]$ancestry.conversion.before,
      Output = variable.summary$variables[[name]]$ancestry.conversion.after,
      Reasoning = variable.summary$variables[[name]]$ancestry.reasoning,
      "Best Match" = variable.summary$variables[[name]]$ancestry.best.match,
      "Best Score" = variable.summary$variables[[name]]$ancestry.best.value,
      "Second Match" = variable.summary$variables[[name]]$ancestry.second.match,
      "Second Score" = variable.summary$variables[[name]]$ancestry.second.value
    )
    rownames(df) <- NULL

...
for (name in names(variable.summary)) {
  variable.summary.df <- data.frame(
    names(variable.summary[[name]]$summary), as.vector(variable.summary[[name]]$summary)
  )
  rownames(variable.summary.df) <- NULL
  colnames(variable.summary.df) <- c("Value", "Summary statistics")
  knitr::kable(variable.summary.df, caption = paste("Summary of ", name, " (", variable.summary[[name]]$original.name, ")", sep = ""))
}
```
